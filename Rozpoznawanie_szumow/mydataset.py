...
class Dataset:
    ...
    def batch_creator(self, mode, batch_size):
        ...
        # Clean speech
        if mode == 'TRAIN':
            clean_signal = self.clean['Clean_train']
            random.shuffle(clean_signal)
        elif mode == 'TEST':
            clean_signal = self.clean['Clean_test']
            random.shuffle(clean_signal)
        elif mode == 'VAL':
            clean_signal = self.clean['Clean_val']
            random.shuffle(clean_signal)
        else:
            sys.exit('Wrong mode')
        ...
        assert len(clean_signal) == len(noise_signal)
        sample_idx = 0

        while sample_idx < len(clean_signal):
            #not equal batches
            if sample_idx > (len(clean_signal)-batch_size):
                samples = len(clean_signal)-sample_idx
            else:
                samples = batch_size

            mixed_signals_spectrograms = np.empty((samples, 2, 256, train_length), dtype=np.float32)
            clean_signals_spectrograms = np.empty((samples, 2, 256, train_length), dtype=np.float32)
            weights = np.empty((samples, train_length), dtype=np.float32)
            ...
            if sample_idx > (len(clean_signal) - batch_size):
                zakres = (len(clean_signal) - sample_idx)
            else:
                zakres = batch_size

            for batch_idx in range(zakres):
                mixed, clean_spec, mixed_spec = self.mix_signals(clean_signal[sample_idx+batch_idx], noise_signal[sample_idx+batch_idx])
                ...
                if mixed_spec.shape[1] > train_length:
                    ...
                    clean_spec = clean_spec[:, :train_length]
                ...
                weights[batch_idx, start_idx:end_idx] = 1.0
    ...
    def mix_signals(self, c_signal, n_signal):
        assert len(n_signal) > len(c_signal)

        start_idx = random.randint(0, len(n_signal) - len(c_signal))
        n_signal = n_signal[start_idx: start_idx+len(c_signal)]

        c_signal_rms = np.sqrt(np.mean(c_signal**2.0))           #RMS
        n_signal_rms = np.sqrt(np.mean(n_signal**2.0))

        snr = 0.0
        g = 10 ** (snr / 20.0) * n_signal_rms / c_signal_rms    #wzmocnienie

        clean_spec = self.spectrogram(g*c_signal)
        noise_spec = self.spectrogram(n_signal)

        mixed_signal = g*c_signal + n_signal
        mixed_spec = clean_spec + noise_spec

        return mixed_signal, clean_spec, mixed_spec
...
if __name__ == "__main__":
    ...
    for batch in obiekt.batch_creator('TRAIN', 5):
        i, o, labels, w = batch
        print(labels)
        print('-------')
    print('koniec train')
